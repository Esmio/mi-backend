...
class Goods extends Service {
  async createNewProduct(good) {
    const created = await this.app.model.Goods.createNewGood(good);
    return created;
  }
  ...
  async fillGoodsInfo(goods) {
    ...
    goods.installment = {
      default_install: 'mifinanceinstal_m',
      list: [
        {
    ...
    // await this.ctx.service.servicebargins.getBarginListByGoodsId(goods.id);
